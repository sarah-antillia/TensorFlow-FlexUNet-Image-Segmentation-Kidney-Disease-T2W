...
#
# 2026/02/04 ImageMaskDatasetGenerator.py
...
class ImageMaskDatasetGenerator:
  ...
  def generate(self, image_file_pattern, mask_file_pattern, output_images_dir, output_masks_dir, color):
    image_files = sorted(glob.glob(image_file_pattern))
    mask_files  = sorted(glob.glob(mask_file_pattern))
    ...
    for i in range(l1):
      index += 1000
      #input("HIT")
      self.generate_mask_files(mask_files[i], index, output_masks_dir, color) 
      self.generate_image_files(image_files[i],index, output_images_dir)
  ...
  def generate_mask_files(self, niigz_file, index, output_masks_dir, color ):
    nii = nib.load(niigz_file)
    fdata  = nii.get_fdata()
    ...
    for i in range(d):
      ...
      if img.any() >0:
        #img = img*255.0
        img = self.colorize_mask(img, color)
        ...
        print("--- Saved {}".format(filepath))
        if self.augmentation:
          self.augment(img, filename, output_masks_dir, border=(0, 0, 0), mask=True)

      else:
        print("=== Skipped {}".format(filepath))

  def colorize_mask(self, segmentation, color):
...
if __name__ == "__main__":
  try:
    image_file_pattern  = "./archive/CKD/*_T2W.nii/*.nii"
    mask_file_pattern   = "./archive/CKD/*_T2W_mask.nii/*.nii"

    output_dir  = "./Augmented-Kidney-Disease-T2W-master/"
    ...
    #BGR red
    color = (0,0,255)
    generator.index = 100000
    generator.generate(image_file_pattern, mask_file_pattern, output_images_dir, output_masks_dir, color)

    image_file_pattern  = "./archive/Healthy_Control/*_T2W.nii/*.nii"
    mask_file_pattern   = "./archive/Healthy_Control/*_T2W_mask.nii/*.nii"

    color = (0,255,0)
    generator.index = 200000
